	...
	$(".admin-account").on('click', function(){
		...
		$.ajax({
			...
			success : function(adminList){
				...
				//var list = '<tr><td>'+json[i].accountId+'</td><td>'+json[i].email+'</td><td><span class="badge">'+json[i].status+'</span></td><td><button type="submit" class="btn-danger btn" name="buttonClickList" value="Invalidate" onclick="invalidAcc(this)" data-id="'+json[i].accountId+'">Invalidate Account</button><button type="submit" class="btn-danger btn" name="buttonClickList" value="RestoreAcc" onclick="restoreAcc(this)" data-id="'+json[i].accountId+'">Restore Account</button></td>'
